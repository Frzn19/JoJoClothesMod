...
public class DioHeadbandArmor extends ItemArmor implements IHasModel {
    public DioHeadbandArmor(String name, CreativeTabs tab, ArmorMaterial material, EntityEquipmentSlot equipmentSlot) {
    ...
    public ModelBiped getArmorModel(EntityLivingBase entityLiving, ItemStack itemStack, EntityEquipmentSlot armorSlot, ModelBiped _default) {
        if(itemStack != ItemStack.EMPTY){
            if(itemStack.getItem() instanceof DioHeadbandArmor){
                DIOHeadBand model = new DIOHeadBand(0.01f);
